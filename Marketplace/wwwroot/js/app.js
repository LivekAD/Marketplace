(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".swiper-product")) new core(".swiper-product", {
            modules: [ Navigation, Scrollbar, Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
    ...
    let photoUpload = document.getElementById("photo-upload"), imgUploadPreview = document.querySelector(".img-upload-preview");
    photoUpload.onchange = function() {
        for (let i = 0; i < photoUpload.files.length; i++) if (false === /\.(jpe?g|png|gif)$/i.test(photoUpload.files[i].name)) {
            alert("this type is not supported");
            photoUpload.value = "";
            break;
        } else {
            let reader = new FileReader, photoFileDetailsTemplate = (photoUpload.files[i].name, 
            Math.round(photoUpload.files[i].size / 1024), photoUpload.files[i].type, `\n        <div class="photo-file-details">\n        </div>`);
            reader.onload = function(event) {
                let previewImage = document.createElement("img"), previewImageBox = document.createElement("div"), removeImage = document.createElement("i"), removeIcon = document.createTextNode("x");
